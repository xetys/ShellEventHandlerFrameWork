...
class Thread {
    ...
    public function setRunnable( $_runnable ) {
        if( $this->runnableOk( $_runnable ) ) {
            if($_runnable == "RUN")
                $_runnable = array($this,"run");
     ...
     * @return boolean
     */
    public function runnableOk( $_runnable ) {
        if($_runnable == "RUN")
            return ( method_exists( $this,"run") && is_callable( array($this,"run") ) );
        else if(is_array($_runnable))
        {
            $obj = $_runnable[0];
            $method = $_runnable[1];
            return ( method_exists( $obj,$method) && is_callable( array($obj,$method) ) );
        }
        else
